...
void shared_sgemm_kernel(float *C, float *A, float *B, long size)
{
	...
	/* DONE: declare shared memory with size TILE_SIZE x TILE_SIZE */
	...
	if (col < size && row < size) {
		...
		for (long m = 0; m < size / TILE_SIZE; ++m) {
			...
			/* DONE: introduce a pragma directive that can potentially improve performance here */
			#pragma unroll
			for (long k = 0; k < TILE_SIZE; ++k) {
				/* DONE: Perform multiplication here */
...
void cublas_sgemm(float *C, float *A, float *B, long size)
{
	...
	gettimeofday(&t0, NULL);
	/* DONE: fill in the blanks, do C = BA instead of C = AB */
	// cublasSgemm(handle, CUBLAS_OP_N, CUBLAS_OP_N, , , , , , , , , , , );
	//                                            h_A   w_B   w_A              w_B      w_A             w_A  
